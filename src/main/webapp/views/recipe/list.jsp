...
<display:table name="recipe" id="row" requestURI="${requestURI }" pagesize="5" class="displaytag">
	...
	<security:authorize access="hasRole('USER')">
		<spring:message code="user.recipe" var="userRecipe"/>
		<display:column title="${userRecipe}">
			<jstl:if test="${row.user == userVar }">
				<a href="user/recipes/edit.do?recipeId=${row.id}">
					<spring:message code="user.recipe.edit"/>
				</a>
			</jstl:if>
		</display:column>
	</security:authorize>
	...
	<security:authorize access="hasRole('USER')">
			<spring:message code="user.recipe.ingredient" var="userIngredient"/>
			<display:column title="${userIngredient}">
					<a href="user/ingredients/list.do?recipeId=${row.id}">
						<spring:message code="user.recipe.ingredient.list"/>
					</a>
			</display:column>
	</security:authorize>
	...
	<security:authorize access="isAnonymous()">
			<spring:message code="user.recipe.ingredient" var="userIngredient"/>
			<display:column title="${userIngredient}">
					<a href="user/ingredients/watch/list.do?recipeId=${row.id}">
						<spring:message code="user.recipe.ingredient.list"/>
					</a>
			</display:column>
	</security:authorize>
	...
	<security:authorize access="isAnonymous()">
			...
			<display:column title="${bar1}">
					<a href="bartols/recipe/list.do?recipeId=${row.id}">
